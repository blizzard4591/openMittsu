...
namespace openmittsu {
	namespace database {
		namespace internal {
			...
			DatabaseReadonlyGroupMessage DatabaseGroupMessageCursor::getReadonlyMessage() const {
				if (!isValid()) {
					throw openmittsu::exceptions::InternalErrorException() << "Can not create message wrapper for invalid message.";
				}
				...
				DatabaseReadonlyGroupMessage drgm(m_group, contact, messageId, isMessageFromUs, createdAt, sentAt, modifiedAt, isQueued, isSent, uuid, isRead, isSaved, messageState, receivedAt, seenAt, isStatusMessage, caption, groupMessageType, body, mediaItem);
				return drgm;
			}
