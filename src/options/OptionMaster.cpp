...
namespace openmittsu {
	namespace options {
		...
		OptionMaster::OptionMaster(openmittsu::database::DatabaseWrapper const& database) : QObject(nullptr), m_database(database) {
		...
		void OptionMaster::onDatabaseOptionsChanged() {
		...
		void OptionMaster::registerOptions(OptionRegister* target, QHash<OptionGroups, QString>& groupsToName) {
			...
			target->registerOption(OptionGroups::GROUP_PRIVACY, Options::BOOLEAN_SEND_TYPING_NOTIFICATION, QStringLiteral("options/sendTypingNotification"), tr("Whether the \"user is typing\" notification should be sent."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_PRIVACY, Options::BOOLEAN_SEND_READ_NOTIFICATION, QStringLiteral("options/sendReadNotification"), tr("Whether the \"message seen\" notification should be sent."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_NOTIFICATIONS, Options::BOOLEAN_FORCE_FOREGROUND_ON_MESSAGE_RECEIVED, QStringLiteral("options/forceForegroundOnMessageReceived"), tr("Whether the app should be forced into the foreground when a new message is received."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_NOTIFICATIONS, Options::BOOLEAN_PLAY_SOUND_ON_MESSAGE_RECEIVED, QStringLiteral("options/playSoundOnMessageReceived"), tr("Whether a sound should be played when a new message is received."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_GENERAL, Options::BOOLEAN_RECONNECT_ON_CONNECTION_LOSS, QStringLiteral("options/reconnectOnConnectionLoss"), tr("Whether a reconnect should automatically be tried on a connection loss."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_GENERAL, Options::BOOLEAN_TRUST_OTHERS, QStringLiteral("options/trustOthers"), tr("Whether to accept messages from users whos group membership has not (yet) been confirmed."), false, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_GENERAL, Options::BOOLEAN_UPDATE_FEATURE_LEVEL, QStringLiteral("options/updateFeatureLevel"), tr("Whether the supported feature level of the used identity should be updated if the stored feature level is lower than the one supported by this app."), true, OptionTypes::TYPE_BOOL, OptionStorage::STORAGE_DATABASE);
			target->registerOption(OptionGroups::GROUP_GENERAL, Options::FILEPATH_DATABASE, QStringLiteral("options/database/databaseFile"), tr("The file path where the main database file is stored."), "", OptionTypes::TYPE_FILEPATH, OptionStorage::STORAGE_SIMPLE);
		...
		QSettings* OptionMaster::getSettings() {
		...
		OptionTypes OptionMaster::getOptionTypeForOption(Options const& option) const {
			if (m_optionToOptionContainerMap.contains(option)) {
		...
		QString OptionMaster::toStringRepresentation(QVariant const& value, OptionTypes const& optionType) {
			if (optionType == OptionTypes::TYPE_BINARY) {
				...
				throw openmittsu::exceptions::InternalErrorException() << "Unknown OptionMaster::OptionTypes Key with value " << static_cast<int>(optionType) << " found!";
			}
		}
		...
		bool OptionMaster::toBoolRepresentation(QString const& value) {
		...
		QString OptionMaster::toQStringRepresentation(QString const& value) {
			return value;
		}
		...
		QByteArray OptionMaster::toQByteArrayRepresentation(QString const& value) {
