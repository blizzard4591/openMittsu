...
MessageContent* ContactLocationMessageContent::fromPacketPayload(FullMessageHeader const& messageHeader, QByteArray const& payload) const {
	...
	if (positionOfNewLine == -1) {
		throw ProtocolErrorException() << "Could not split payload of a ContactLocationMessageContent, it does not contain the 0x0A LF splitter.";
	}
	...
	if (splitPositions.size() != 3) {
		throw ProtocolErrorException() << "Could not split payload of a ContactLocationMessageContent, it contains " << splitPositions.size() << " instead of 3 positions. Input String: " << positions.toStdString();
	}
