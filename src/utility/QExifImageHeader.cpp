...
public:
    QExifAsciiValuePrivate( const QString &v )
        : QExifValuePrivate( QExifValue::Ascii, v.toUtf8().size() + 1 ), value( v )
    {}
...
quint32 QExifImageHeader::sizeOfData(const QExifValue &value) const
{
    switch (value.type()) {
...
quint32 QExifImageHeader::calculateSize(const QMap<T, QExifValue> &values) const
{
	...
	size += sizeof(quint32); // The Link-Offset

    return size;
}

template <typename T>
quint32 QExifImageHeader::calculateDataSize(const QMap<T, QExifValue> &values) const {
	quint32 size = 0;

	foreach(const QExifValue &value, values) {
		size += sizeOfData(value);
	}
...
qint64 QExifImageHeader::size() const
{
    if (d->size == -1) {
        d->size
                ...
                + 2                                    // Marker
                + 4                                    // Image Ifd offset
				+ 2									   // Number of Directory Entries in IFD0
...
QByteArray QExifImageHeader::extractExif(QIODevice *device) const {
	...
	while (true) {
		...
		stream >> markerA;
		if ((markerA != 0xFF) || stream.atEnd()) {
			return QByteArray();
		} else {
			stream >> markerB;
			if (markerB == 0xD9) { // End of Image
				return QByteArray();
			} else if (markerB == 0xDD) { // Define Restart Interval, 4Byte payload
				device->read(4);
				continue;
			} else if (markerB == 0xDA) { // Start of Scan, contains no usable length information, can not be skipped without parsing the data
				return QByteArray();
			} else if ((markerB >= 0xD0) && (markerB <= 0xD7)) { // Restart Marker, no Payload
				continue;
			} else if (markerB == 0xE1) { // EXIF Marker
				break;
			} else {
				// This should be a payload-frame
				quint16 length;
				stream >> length;
				if (!device->seek(device->pos() + length - 2)) {
					return QByteArray();
				}
			}
		}
...
quint32 QExifImageHeader::writeExifValue(QDataStream &stream, const QExifValue &value) const
{
...
template <typename T> quint32 QExifImageHeader::writeExifValues(QDataStream &stream, const QMap<T, QExifValue> &values) const {
	quint32 bytesWritten = 0;
	for (typename QMap<T, QExifValue>::const_iterator i = values.constBegin(); i != values.constEnd(); i++) {
		bytesWritten += writeExifValue(stream, i.value());
	}
...
qint64 QExifImageHeader::write(QIODevice *device) const {
	...
	// 8 Bytes Header, 2 Bytes ImageIfd values.count, 12 Bytes ExifIfd Header, 4 Bytes Thumbnail Offset
    quint32 offset = 8 + sizeof(count) + 12 + sizeof(quint32); // should be: 26
    ...
    stream << count; 
	bytesWritten += 2; // 8 + 2 = 10 Bytes written in total
	...
	bytesWritten += 12; // another 12 Bytes written

	quint32 const exifIfdValuesDataOffset = calculateDataSize(d->exifIfdValues);
    offset += calculateSize(d->exifIfdValues);

    quint32 gpsIfdOffset = offset;
	quint32 gpsIfdValuesDataOffset = 0;
    ...
    if (!d->gpsIfdValues.isEmpty()) {
		...
		// Requires 12 Bytes of additional storage
        //d->imageIfdValues.insert(ImageTag(GpsInfoIfdPointer), QExifValue(offset));

		gpsIfdValuesDataOffset = calculateDataSize(d->gpsIfdValues);
        offset += calculateSize(d->gpsIfdValues);
    }

	// Another 4 Bytes (End of Link in IFD0 or Link to IFD1)
	if (!d->thumbnailData.isEmpty()) {
	...
	quint32 correctedOffset = (offset - calculateSize(d->exifIfdValues)) + sizeof(quint16) + sizeof(quint32);
	quint32 exifSubIfdSize = writeExifHeaders(stream, d->exifIfdValues, correctedOffset) - correctedOffset;
	quint32 dataOffsetExifIfdValues = exifSubIfdSize - (d->exifIfdValues.count() * 12);
	bytesWritten += (d->exifIfdValues.count() * 12);

	// The End-of-Link for the Exif Sub-IFD
	stream << quint32(0);
	bytesWritten += sizeof(quint32);
    ...
    Q_ASSERT(startPos + gpsIfdOffset == currentPos);
	Q_ASSERT(startPos + bytesWritten == currentPos);
