 ...
 */
bool Gzip::gzipCompress(QByteArray const& input, QByteArray& output, int level) {
	// Prepare output
	output.clear();
	...
	if (input.length()) {
		...
		// Initialize deflater
		int ret = deflateInit2(&strm, qMax(-1, qMin(9, level)), Z_DEFLATED, GZIP_WINDOW_BITS, 8, Z_DEFAULT_STRATEGY);
		...
		char const* input_data = input.data();
 ...
 */
bool Gzip::gzipDecompress(QByteArray const& input, QByteArray& output) {
	// Prepare output
	output.clear();
