...
private:
	...
	quint32 writeExifValue(QDataStream &stream, const QExifValue &value) const;

    template <typename T> quint32 writeExifHeaders(
            QDataStream &stream, const QMap<T, QExifValue > &values, quint32 offset) const;
    template <typename T> quint32 writeExifValues(
            QDataStream &target, const QMap<T, QExifValue> &values) const;

	quint32 sizeOfData(const QExifValue &value) const;
    quint32 sizeOf(const QExifValue &value) const;

    template <typename T> quint32 calculateSize(const QMap<T, QExifValue> &values) const;
	template <typename T> quint32 calculateDataSize(const QMap<T, QExifValue> &values) const;
